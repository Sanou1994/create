...
constructor(props){
    super(props);
    this.state = {render:'',caisse1:[],caisse2:[],caisse:[],isLoad:false,url:'http://localhost:8080/api/',
    ...
    content:'',pri:'',optrImprime:"",opImprime:"",montantImprime:"",fraisImprime:""
...
componentDidMount(){
    ...
    var idU=JSON.parse(localStorage.getItem('logaccorde'))[0].id;
    this.handleClick();
    const apiUrl = 'http://localhost:8080/api/transactions/caissier/'+idU;
    var somme=0;
    ...
    fetch(apiUrl)
     ...
     var somme2=0;
     const apiUrl2 = 'http://localhost:8080/api/soldeDebuterJournees/caissier/'+idU;
     fetch(apiUrl2)
     .then((response) => response.json())
     .then((data) => {console.log('This is your data debute ', data[0])
       if( typeof(data[0])!="undefined") {
        this.setState({sommeInitial:data[0].sommeInitiale});
...
handledelete(){
    console.log("ok...........!!!! id: "+this.state.id);
        axios.delete('http://localhost:8080/api/transactions/supprimer/'+this.state.id)
        .then(
            res=>{
...
handleTrieoperateur(){
    ...
    const apiUrl = 'http://localhost:8080/api/transactions/caissier/'+idU+'/recherche/operateur/'+$("#trie option:selected").text();
    var somme=0;
    var sommeDecaissement=0;
    var sommeEncaissement=0;
    var sommeComm=0;
...
handleTrieoperation(){
    ...
    const apiUrl = 'http://localhost:8080/api/transactions/caissier/'+idU+'/recherche/operation/'+$("#trieoperation option:selected").text();
   var somme = 0;
   var sommeDecaissement=0;
   var sommeEncaissement=0;
   var sommeComm=0;
 ...
 printDiv=()=> 
{
    ...
    //  newWin.document.body.style.backgroundColor  ='#F31212';
    //  newWin.document.open();
     

    //  newWin.document.write('<html><body  onload="window.print()"><div style="width:200px;height:200px">'+
...
modele=()=>{
    ...
    if(this.state.checked){
        ...
        return (
           ...
           {<ModalDebuter/>}
        <div class="card text-dark bg-light mt-3" id="card1">
            ...
            <div class="card-body ">
                ...
                <form class="form-inline right">
                    ...
                    <div calssName="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                        <h4 className="mr-lg-5" >{this.state.date}</h4>
                    </div>

                    <div calssName="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                        <a id="demarre" href="/#/encaissement" className="btn btn-lg btn-outline-success float-right ml-lg-5 mr-2">Encaisser</a>
                    </div>

                    <div calssName="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                        <a href="/#/decaissement" className="btn btn-lg btn-outline-danger float-right m-2">Décaisser</a>
                    </div>
                </form>
